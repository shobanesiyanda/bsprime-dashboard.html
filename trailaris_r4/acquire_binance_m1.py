...
OUT=Path('trailaris_r4/raw_m1');OUT.mkdir(parents=True,exist_ok=True)
...
START=date(2026,5,4);END=date(2026,8,12);cov={}
def fetch(u):return urllib.request.urlopen(urllib.request.Request(u,headers={'User-Agent':'TrailarisResearch/R4'}),timeout=60).read()
def ziprows(raw):
...
for asset,sym in ASSETS.items():
 ...
 with p.open('w',newline='') as f:
  ...
  for y,m in [(2026,5),(2026,6),(2026,7)]:
   stem=f'{sym}-1m-{y:04d}-{m:02d}.zip';url=f'{BASE}/monthly/klines/{sym}/1m/{stem}'
  ...
  while d<=END:
   ...
   d+=timedelta(days=1)
 cov[asset]={'source':'BINANCE_OFFICIAL_SPOT_M1','rows':n,'first':first,'last':last,'status':'OK' if n>100 else 'FAIL','errors':errs[:10]};print(asset,n,len(errs))
Path('trailaris_r4/coverage_m1_binance.json').write_text(json.dumps(cov,indent=2))
