...
from trailaris_50_asset.batch_consistent_drawdown import batch_consistent_event_drawdown
...
def main():
    ...
    legacy_event_dd=float(m['max_event_equity_drawdown_pct']);dd=batch_consistent_event_drawdown(W,EV,fcache,100.0);m['legacy_global_event_drawdown_pct']=legacy_event_dd;m['max_event_equity_drawdown_pct']=float(dd['max_event_equity_drawdown_pct']);m['batch_consistent_event_drawdown_pct']=float(dd['max_event_equity_drawdown_pct']);m['batch_consistent_worst_episode']=dd['worst_episode'];m['event_drawdown_authority']='BATCH_CONSISTENT_WITHIN_EACH_WEEK_LOCAL_REPLAY_BOOK'
    if len(EV):
