...
const PACE_MS=Number(arg('--pace-ms','4000'));
...
async function requestPage(client,symbol,endEpoch,stats){
 ...
 for(let attempt=1;attempt<=15;attempt++){
  try{
   const x=await client.req({ticks_history:symbol,end:String(endEpoch),style:'candles',granularity:60,count:PAGE_COUNT,adjust_start_time:1,subscribe:0});
   stats.requests++;await sleep(PACE_MS);return x;
  }catch(e){
   last=String(e);
   if(last.includes('RateLimit')){stats.rate_limit_backoffs++;const ms=Math.min(180000,30000*Math.max(1,stats.rate_limit_backoffs));console.error(JSON.stringify({symbol,event:'RATE_LIMIT_BACKOFF',attempt,ms,error:last}));await sleep(ms)}
   else{console.error(JSON.stringify({symbol,event:'REQUEST_RETRY',attempt,error:last}));await sleep(Math.min(20000,1500*attempt))}
  }
 }
 ...
 try{
  ...
  while(true){
   ...
   try{x=await requestPage(client,symbol,endEpoch,stats)}catch(e){
    try{client.ws.close()}catch{};stats.reconnects++;await sleep(15000);client=await openClient();x=await requestPage(client,symbol,endEpoch,stats);
   }
   ...
   for(const c of cs){const t=(+c.epoch)*1000;if(!Number.isFinite(t)||t<S||t>=E)continue;const o=+c.open,h=+c.high,l=+c.low,cl=+c.close;if(!valid(o,h,l,cl)){stats.invalid++;continue}const sig=[o,h,l,cl].join('|');if(map.has(t)){stats.duplicates++;if(map.get(t).sig!==sig)stats.conflicts++;continue}map.set(t,{sig,row:[new Date(t).toISOString(),o,h,l,cl,'','','',0,'DERIV_OFFICIAL_M1_CANDLES',asset]})}
  ...
  return {instrument_id:symbol,canonical_instrument:asset,provider_name:'DERIV_OFFICIAL_M1_CANDLES',rows:rows.length,first:first?first.toISOString():'',last:last?last.toISOString():'',full_window_coverage:coverage,status:ok?'PASS':'FAIL',file:ok?path.join(OUT,asset+'_M1_normalized.csv'):'',...stats,page_count:PAGE_COUNT,pace_ms:PACE_MS,request_semantics:'BACKWARD_COUNT_5000_CANDLE_PAGES__ONE_SYMBOL_AT_A_TIME__GLOBAL_PACING__RATE_LIMIT_BACKOFF__NO_SUBSCRIPTION'};
