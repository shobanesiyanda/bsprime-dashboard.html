...
def main():
    ...
    P=pd.concat(promoted,ignore_index=True) if promoted else pd.DataFrame();P.to_csv(a.outdir/f'QV2_50_PROMOTED_SHARD_{a.shard}.csv.gz',index=False,compression='gzip')
    status={'state':'QV2_50_WEEKLY_PROMOTION_SHARD_COMPLETE','shard':a.shard,'shards':a.shards,'candidate_rows':len(cands),'candidate_assets':int(cands.asset.nunique()),'assigned_week_count':len(assigned),'weeks':counts,'promoted_rows':len(P),'strategy_blob':'120390ab52e80f51bafedc42389ceb7420a410f2','decision_semantics_changed':False}
    (a.outdir/f'QV2_50_PROMOTION_SHARD_{a.shard}_STATUS.json').write_text(json.dumps(status,indent=2,default=str));print(json.dumps(status,indent=2,default=str))
