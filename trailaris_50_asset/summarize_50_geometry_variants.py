...
def batch_consistent_event_dd(ev):
    ...
    i=pd.to_numeric(y.drawdown,errors='coerce').idxmin()
    r=y.loc[i]
...
def summarize_one(mode,d,control):
    ...
    corrected_dd,episode=batch_consistent_event_dd(ev)
    core=ev[ev.universe_role=='CORE34'];add=ev[ev.universe_role=='EXPANSION16']
    win=int((ev.net_pnl>0).sum());loss=int((ev.net_pnl<0).sum());flat=int((ev.net_pnl==0).sum())
    reasons=de.reason.astype(str).value_counts().to_dict()
    c34=control['realized_closed_balance_replay']
    return {
      'mode':mode,'quant_v2_modified':False,'promotion_estate_modified':False,
      'portfolio_overlay_only':mode!='CONTROL,
      'end_balance_usd':float(m['end_balance']),'return_pct':float(m['compounded_return_pct']),
      'positive_weeks':int(m['positive_weeks']),'weeks_ge5':int(m['weeks_ge5']),'weeks_ge10':int(m['weeks_ge10']),
      'trades':int(len(ev)),'wins':win,'losses':loss,'flats':flat,'profit_factor':pf(ev),
      'reported_event_dd_pct':float(m['max_event_equity_drawdown_pct']),
      'batch_consistent_event_dd_pct':corrected_dd,'batch_consistent_worst_episode':episode,
      'max_weekly_dd_pct':float(m['max_weekly_drawdown_pct']),
      ...
      'tier_attribution':group_stats(ev,'promotion_tier'),'family_attribution':group_stats(ev,'strategy_family'),'class_attribution':group_stats(ev,'asset_class'),
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--root',type=Path,required=True);ap.add_argument('--control',type=Path,required=True);ap.add_argument('--out',type=Path,required=True);a=ap.parse_args()
    control=json.loads(a.control.read_text());modes=['CONTROL','FACTOR100','WEEK8','FACTOR100_WEEK8'];rows=[]
    for mode in modes:
        d=a.root/mode
        if not (d/'QV2_50_REPLAY_STATUS.json').exists():continue
        rows.append(summarize_one(mode,d,control))
    by={r['mode']:r for r in rows};base=by.get('CONTROL')
    if base:
        for r in rows:
            r['delta_vs_control']={'end_balance_usd':r['end_balance_usd']-base['end_balance_usd'],'profit_factor':r['profit_factor']-base['profit_factor'],'batch_consistent_event_dd_pct_points':r['batch_consistent_event_dd_pct']-base['batch_consistent_event_dd_pct'],'trades':r['trades']-base['trades']}
    ...
    a.out.parent.mkdir(parents=True,exist_ok=True);a.out.write_text(json.dumps(report,indent=2,default=str))
    flat=[]
