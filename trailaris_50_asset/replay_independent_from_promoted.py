...
from trailaris_50_asset.batch_consistent_drawdown import batch_consistent_event_drawdown,batch_consistent_global_event_drawdown,legacy_global_event_drawdown_pct
...
def main():
 ...
 equity=100.0;Wrows=[];events=[];decisions=[];raw_start=min(pd.Timestamp(x.timestamp.iloc[0]) for x in fcache.values());raw_end=max(pd.Timestamp(x.timestamp.iloc[-1]) for x in fcache.values())
 for wk in weeks:
 ...
 status={'state':'QV2_INDEPENDENT_26W_REPLAY_COMPLETE','assets':a.assets,'raw_observed_start':str(raw_start),'raw_observed_end':str(raw_end),'evaluated_first_week':str(weeks[0]),'evaluated_last_week':str(weeks[-1]),'evaluated_end_exclusive':str(weeks[-1]+pd.Timedelta(days=7)),'strategy_blob':'120390ab52e80f51bafedc42389ceb7420a410f2','closed_balance_compounding':True,'quant_v2_modified':False,'asset_identity_modified':False,'candidate':m,'core34':stats(core),'expansion16':stats(add) if a.assets==50 else None,'drawdown_reporting':'AUTHORITATIVE FULL-PERIOD DD = BATCH-CONSISTENT REPLAY EVENT ROW ORDER / balance_after / GLOBAL PEAK-TO-TROUGH. WEEK-LOCAL MARK-TO-MARKET AND LEGACY TIMESTAMP-RESORTED VALUES ARE DIAGNOSTICS ONLY.','evidence_class':'INDEPENDENT_HISTORICAL_26_WEEK_CAUSAL_WALK_FORWARD_RESEARCH_PROXY; BROKER_PRODUCTION_CERTIFICATION_SEPARATE'}
