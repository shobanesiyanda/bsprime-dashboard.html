...
OUT=Path(os.environ.get('FORWARD_RAW','trailaris_r5_5/forward_holdout/raw'));OUT.mkdir(parents=True,exist_ok=True)
...
START=date.fromisoformat(os.environ['FORWARD_START']);END=date.fromisoformat(os.environ['FORWARD_END']);cov={}
if END<START:raise SystemExit('invalid forward range')
def fetch(u):return urllib.request.urlopen(urllib.request.Request(u,headers={'User-Agent':'TrailarisResearch/R5.5-Forward'}),timeout=60).read()
def ziprows(raw):
...
def addzip(w,url,asset):
 ...
 for r in ziprows(raw):
  ...
  if START<=d<=END:w.writerow([iso(t),r[1],r[2],r[3],r[4],'','','',r[5],'BINANCE_OFFICIAL_SPOT_M1_FORWARD_HOLDOUT',asset]);n+=1;first=t if first is None else first;last=t
 return n,first,last
for asset,sym in ASSETS.items():
 ...
 with p.open('w',newline='') as f:
  ...
  while d<=END:
   ...
   d+=timedelta(days=1)
 cov[asset]={'source':'BINANCE_OFFICIAL_SPOT_M1_FORWARD_HOLDOUT','rows':n,'first':first,'last':last,'status':'OK' if n>100 else 'FAIL','errors':errs[:20]};print(asset,n,len(errs))
Path(OUT/'coverage_crypto.json').write_text(json.dumps(cov,indent=2))
