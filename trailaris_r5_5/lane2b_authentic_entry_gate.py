...
class EntryGateVariant:
    ...
    def promote_for_week(self,cands,week_start):
        ...
        if len(pr):
            ...
            pr=pr.loc[~m].copy()
            pr['r5_5_entry_gate']=self.rule.get('rule_id','LANE2B')
...
def main():
    ...
    if passing<1:
        status={'state':'R5_5_LANE2B_NOT_RUN_NO_PASSING_ENTRY_GATE','passing_rules':passing,'next_lane':'ASSET_STRATEGY_REGIME_THESIS_REPAIR','governance':'No selection-frozen gate is forced into authentic replay without first8 discovery plus untouched last3 validation.'}
    ...
    fresh_delta=float(cand['fresh_return_pct'])-float(ctl['fresh_return_pct']);fresh_ok=fresh_delta>1e-9
    dd_ok=float(cand['max_weekly_dd_pct'])>=float(ctl['max_weekly_dd_pct'])-1e-9
    losses_ok=int(cand['losses'])<int(ctl['losses'])
    flats_ok=int(cand['flat'])<=int(ctl['flat'])
    ...
    promo=bool(end_ok and fresh_ok and dd_ok and losses_ok and flats_ok and winrate_ok and routes_ok and asset_ok)
    status={'state':'R5_5_LANE2B_AUTHENTIC_ENTRY_GATE_COMPLETE','evidence_class':'END_TO_END_CAUSAL_FULL_REPLAY','selected_lane2a_rule':rule,'r5_4_control':ctl,'candidate':cand,'delta_end_equity':float(cand['end_equity'])-float(ctl['end_equity']),'fresh_delta_pct':fresh_delta,'gates':{'end_equity_not_regressed':end_ok,'fresh_holdout_improved':fresh_ok,'drawdown_not_worse':dd_ok,'losses_reduced':losses_ok,'flats_not_worse':flats_ok,'nonflat_win_rate_not_worse':winrate_ok,'selected_asset_coverage_not_worse':asset_ok,'full_universe_34x510':routes_ok},'promotion_candidate':promo,'approved_scope':scope,'governance':'The Lane 2A threshold is frozen from first8 discovery and untouched last3 validation. It acts only on promoted candidates using decision-time fields. R5.4 reliability selection and replay logic remain unchanged. Broker execution degradation remains mandatory before promotion.'}
