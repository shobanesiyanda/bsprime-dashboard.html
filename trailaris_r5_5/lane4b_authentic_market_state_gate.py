...
class MarketStateVariant:
    def __init__(self,rule,promote,base,fcache):self.rule=rule;self.promote=promote;self.base=base;self.fcache=fcache
    def promote_for_week(self,cands,week_start):
        pr,conf,stats=self.promote(cands,week_start,'combined')
        if len(pr):
            z=enrich_market_state(pr,self.fcache);m=rule_mask(z,self.rule);pr=z.loc[~m].copy();pr['r5_5_market_state_gate']=self.rule.get('rule_id','LANE4B')
