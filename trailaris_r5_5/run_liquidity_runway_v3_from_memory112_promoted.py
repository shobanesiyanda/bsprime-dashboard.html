...
def _robust_matrix(hist, row, feature_cols):
    ...
    q1=np.nanquantile(X,.25,axis=0); q3=np.nanquantile(X,.75,axis=0)
...
def knn_predict(hist, row, feature_cols, targets):
    if len(hist)<12:
        ...
        out["neighbors"]=int(len(hist)); out["confidence"]=float(min(1.0,len(hist)/50))
        return out
    X,q,med,scale=_robust_matrix(hist,row,feature_cols)
    d=np.sqrt(np.nanmean(((X-q)/scale)**2,axis=1))
    same_as=(hist["asset"].astype(str).to_numpy()==str(row.get("asset",""))) & (hist["strategy"].astype(str).to_numpy()==str(row.get("strategy","")))
    ...
    for t in targets:
        ...
        if not ok.any():
            ...
            local=float(np.average(y[ok],weights=w[ok])); n=int(ok.sum())
...
def liq_state(score):
    if score>=.72:return "ABUNDANT_SUPPORTIVE"
    if score>=.58:return "SUPPORTIVE"
    if score>=.44:return "ADEQUATE"
    if score>=.32:return "THIN"
...
def entry_assessment(hist, r):
    ...
    if exp_net < 0 and p075 < .48 and eee < .90 and liq < .44:
        cls="REJECT_INSUFFICIENT_RUNWAY"; mult=0.0
    elif p075 >= .56 and (p150 >= .44 or eee >= 1.50) and liq >= .38:
        cls="RUNNER"; mult=1.0
    else:
        cls="EXTRACTION"; mult=.70 if p075>=.50 else .50
    return {
...
def continuation_assessment(hist_enriched, r, cp):
    ...
    if len(h)==0:
        return {"p_continue_1_50r":0.5,"expected_remaining_excursion_r":0.5,"continuation_confidence":0.0,"liquidity_score":0.5,"liquidity_state":"ADEQUATE"}
    rename={f"cp_{key}_{k}":k for k in NUMERIC_ENTRY_FEATURES}
    rename[f"cp_{key}_elapsed_minutes"]="elapsed_minutes"
    hh=h.rename(columns=rename)
...
def target_bank_fraction(runway_class, cp, cont):
    p=cont["p_continue_1_50r"]; liq=cont["liquidity_score"]; rem=cont["expected_remaining_excursion_r"]
    rem_support=float(np.clip(rem/max(1.50-cp,0.25),0,1))
    score=.60*p+.25*liq+.15*rem_support
    if score>=.72: target=0.00
    elif score>=.58: target=0.10
    elif score>=.45: target=0.25
    elif score>=.30: target=0.40
    else: target=0.60
...
def apply_adaptive_banking(r, hist):
    old_gross=_safe(r.get("gross_r", _safe(r.get("net_r",0)) + _safe(r.get("cost_r",0))))
    total_bank=0.0; bank_value=0.0; trace=[]
    for cp in CHECKPOINTS:
        key=str(cp).replace(".","_")
        if not bool(r.get(f"cp_{key}_reached",False)):
            continue
        cont=continuation_assessment(hist,r,cp)
        target,score=target_bank_fraction(str(r.get("campaign_runway_class","EXTRACTION")),cp,cont)
        target=max(total_bank,target)
        inc=max(0.0,target-total_bank)
        if inc>0:
            bank_value += inc*cp; total_bank += inc
        trace.append({"checkpoint_r":cp,"continuation_score":score,"target_bank_fraction":target,"incremental_bank_fraction":inc,**cont})
    out=r.copy()
    ...
    out["gross_r"]=float(new_gross); out["net_r"]=float(new_gross-cost)
...
def metrics(EV,DE,W,roll):
    ...
    return {
        "end_equity":float(W.end_equity.iloc[-1]),"compounded_return_pct":float((W.end_equity.iloc[-1]/100-1)*100),
        ...
        "loss_rate":float(losses/len(EV)) if len(EV) else 0.0,"flat_rate":float(flats/len(EV)) if len(EV) else 0.0,
        "nonflat_win_rate":float(wins/(wins+losses)) if wins+losses else 0.0,"profit_factor":float(gw/gl) if gl else 999.0,
        ...
        "rolling_11w_windows":int(len(roll)),"rolling_11w_min_return_pct":float(roll.compounded_return_pct.min()),
        "rolling_11w_median_return_pct":float(roll.compounded_return_pct.median()),
        "rolling_11w_mean_return_pct":float(roll.compounded_return_pct.mean()),"rolling_11w_max_return_pct":float(roll.compounded_return_pct.max()),
    }
...
def main():
    ...
    ap.add_argument("--rawdir",required=True); ap.add_argument("--specs",required=True); ap.add_argument("--promoted",required=True)
    ap.add_argument("--factory-audit",required=True); ap.add_argument("--outdir",required=True)
    ...
    if len(fa)!=510 or int(fa.strategy_family.nunique())!=15: raise RuntimeError("factory hard lock failed")
    ...
    if len(PR0)!=3417 or len(weeks)!=26 or int(PR0.asset.nunique())!=34: raise RuntimeError("memory112 estate hard lock failed")
    required={"mfe_r","net_r","stop_distance","entry_price","direction","quality","expected_r","reliability_score","reliability_asset_strategy_mean_r","rank_score","cost_r","strategy_family","strategy"}
    missing=required-set(PR0.columns)
    if missing: raise RuntimeError(f"promotion estate missing required causal/training fields: {sorted(missing)}")
    ...
    equity=100.0; Wrows=[]; events=[]; decisions=[]; kept_rows=[]; rejected_rows=[]; assessment_rows=[]
    for wk in weeks:
        ...
        if len(v2_rejected):
            rr=v2_rejected.copy(); rr["r5_5_v3_gate_reason"]="V2_NEGATIVE_EDGE_GATE"; rejected_rows.append(rr.assign(eval_week=wk))
        assessed=[]
        for _,r in v2_kept.iterrows():
            a1=entry_assessment(hist,r)
            q=r.copy()
            for k,v in a1.items(): q[k]=v
            ...
            if a1["campaign_runway_class"]=="REJECT_INSUFFICIENT_RUNWAY":
                q["r5_5_v3_gate_reason"]="RUNWAY_REJECT"; rejected_rows.append(pd.DataFrame([q])); continue
            q["risk_fraction"]=_safe(q.get("risk_fraction",.005))*a1["runway_risk_multiplier"]
            q=apply_adaptive_banking(q,hist)
            assessed.append(q)
        ...
        if len(kept):
            ...
            events.append(ev.assign(eval_week=wk)); decisions.append(de.assign(eval_week=wk)); kept_rows.append(kept.assign(eval_week=wk))
        else:
        ...
        if len(w): rec=w.iloc[0].to_dict(); equity=float(rec["end_equity"])
            ...
            rec=dict(week_start=wk,start_equity=prev,end_equity=prev,weekly_return_pct=0.0,campaigns_entered=0,campaigns_closed=0,max_drawdown_pct=0.0,ge5=False,ge10=False); equity=float(prev)
        rec["source_promoted"]=int(len(src)); rec["v3_promoted_after_all_gates"]=int(len(kept)); Wrows.append(rec)
    ...
    PR=pd.concat(kept_rows,ignore_index=True) if kept_rows else pd.DataFrame(); RJ=pd.concat(rejected_rows,ignore_index=True) if rejected_rows else pd.DataFrame(); AS=pd.DataFrame(assessment_rows)
    roll=rolling(W); cand=metrics(EV,DE,W,roll)
    ...
    cls=PR.get("campaign_runway_class",pd.Series(dtype=str)).value_counts().to_dict() if len(PR) else {}; liq=PR.get("liquidity_sufficiency_state",pd.Series(dtype=str)).value_counts().to_dict() if len(PR) else {}
    status={
        "state":"R5_5_LIQUIDITY_RUNWAY_V3_26W_REPLAY_COMPLETE","baseline":V2_BASELINE,
        "scope":{"routes":34,"strategy_families":15,"route_strategy_cells":510,"weeks":26},
        "source_promotion_estate":"EXACT_R5_5_MEMORY112_PROMOTED_3417","feature_cache_source":"EXACT_R4_BUILD_ASSET_COMPONENTS_FROM_PRESERVED_34_ROUTE_M1_CACHE",
        "candidate":cand,"delta_candidate_minus_locked_v2":delta(cand,V2_BASELINE),
        "architecture":{"entry_continuation_separated":True,"runway_model":"CAUSAL_112D_REGULARIZED_KNN_FROM_PRIOR_EXITED_PROMOTIONS","direct_liquidity":"NOT_AVAILABLE_IN_HISTORICAL_ESTATE","liquidity_model":"CAUSAL_PRICE_VOLATILITY_IMPULSE_EFFICIENCY_RANGE_FRICTION_PROXY","developed_position_checkpoints_r":list(CHECKPOINTS),"adaptive_banking":"0_TO_60_PERCENT_CUMULATIVE_BY_CONTINUATION_STATE","runner":"UNRESTRICTED_REMAINDER; NO_GLOBAL_3R_CEILING","capacity_credit_after_partial":False,"v2_negative_edge_gate_preserved":True},
        "counters":{"entry_assessments":int(len(AS)),"promoted_after_all_gates":int(len(PR)),"rejected_total":int(len(RJ)),"runway_classes":{str(k):int(v) for k,v in cls.items()},"liquidity_states":{str(k):int(v) for k,v in liq.items()},"adaptively_banked_selected_trades":int(mg.sum()) if len(mg) else 0},
        ...
        "automatic_baseline_promotion":False,"evidence_class":"FULL_26_WEEK_CAUSAL_WALK_FORWARD_RESEARCH_PROXY; LIQUIDITY_IS_PROXY_NOT_DIRECT_BOOK; BROKER_PRODUCTION_CERTIFICATION_SEPARATE"
    }
    prefix="R5_5_LIQUIDITY_RUNWAY_V3"
    W.to_csv(out/f"{prefix}_WEEKLY.csv",index=False); roll.to_csv(out/f"{prefix}_ROLLING_11W.csv",index=False); EV.to_csv(out/f"{prefix}_EVENTS.csv",index=False); DE.to_csv(out/f"{prefix}_DECISIONS.csv",index=False); PR.to_csv(out/f"{prefix}_PROMOTED.csv",index=False); RJ.to_csv(out/f"{prefix}_REJECTED.csv",index=False); AS.to_csv(out/f"{prefix}_ENTRY_ASSESSMENTS.csv",index=False)
    (out/f"{prefix}_STATUS.json").write_text(json.dumps(status,indent=2,default=str)); print(json.dumps(status,indent=2,default=str))
