...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--rawdir',required=True);ap.add_argument('--specs',required=True);ap.add_argument('--rule-dir',required=True);ap.add_argument('--outdir',required=True);a=ap.parse_args();out=Path(a.outdir);out.mkdir(parents=True,exist_ok=True);rd=Path(a.rule_dir)
    ...
    status={'state':'R5_5_LANE6_PATH_TARGETED_MATRIX_COMPLETE','evidence_class':'SINGLE_BUILD_POST_RELIABILITY_PRE_SELECTION_PATH_SPECIFIC_FULL_PORTFOLIO_REPLAYS','r5_4_control':ctl,'lane1b2_control':b3,'certified_lane5_control':lane5,'paths':details,'leader':leader,'frontier_candidates':int(df.frontier_pass.sum()) if len(df) else 0,'scope':scope,'next_action':('RUN_PARAMETER_ROBUSTNESS_AROUND_FRONTIER_LEADER' if len(df) and bool(df.frontier_pass.any()) else 'USE_LANE6E_FRESH_STATE_DIAGNOSIS_FOR_CAUSAL_REPAIR'),'governance':'Each path uses its own pre-Aug13 path-specific rule, not a generic winner rule. Winner qualification occurs only after exact causal weekly promotion/reliability scoring and before portfolio selection. The universe is built once and shared across all path tests; this changes compute efficiency only, never market logic. The first pass uses one conservative mechanism setting per path. Parameter expansion is allowed only around a path that shows causal value.'}
