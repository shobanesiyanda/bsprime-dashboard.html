...
def main():
    ...
    emitted_baseline_families=set(cands.strategy_family.astype(str).dropna().unique())
    ...
    emitted_candidate_families=set(c2.strategy_family.astype(str).dropna().unique())
    # 'off' variants are explicit diagnostics. They cannot be successor candidates because the user requires rebuilding, not deleting, both playbooks.
    both_rebuilds_enabled=(cfg['sd']!='off' and cfg['mr']!='off')
    ...
    gates={'end_equity_not_regressed':end_delta>=-1e-9,'aug13_14_verification_not_regressed':fresh_delta>=-1e-9,'drawdown_not_worse':float(cand['max_weekly_dd_pct'])>=float(ctl['max_weekly_dd_pct'])-1e-9,'losses_not_worse':int(cand['losses'])<=int(ctl['losses']),'flats_not_worse':int(cand['flat'])<=int(ctl['flat']),'both_negative_playbooks_rebuilt_not_deleted':both_rebuilds_enabled,'approved_universe_preserved':approved_scope_ok,'full_universe_34x510':len(data)==34 and approved_scope_ok}
    status={'state':'R5_5_LANE3_STRATEGY_REBUILD_V2_FULL_REPLAY_COMPLETE','evidence_class':'END_TO_END_CAUSAL_REBUILT_STRATEGY_REPLAY','variant_index':a.variant_index,'config':cfg,'r5_4_control':ctl,'candidate':cand,'delta_end_equity':end_delta,'aug13_14_verification_delta_pct':fresh_delta,'rebuilt_candidates':int(len(rebuilt)),'rebuilt_raw_stats':stats(rebuilt),'generated_candidate_activity':{'baseline_families_emitting':len(emitted_baseline_families),'candidate_families_emitting':len(emitted_candidate_families),'note':'Historical emission counts are activity only and cannot redefine the approved 15-family/510-cell research universe.'},'gates':gates,'diagnostic_only':not both_rebuilds_enabled,'candidate_pass':bool(both_rebuilds_enabled and all(gates.values())),'promotion_candidate':False,'promotion_blockers':['NEW_UNSEEN_FORWARD_HOLDOUT_REQUIRED','TARGET_SERVER_BROKER_EXECUTION_CERTIFICATION_REQUIRED'],'approved_scope':scope,'governance':'The two negative playbooks are rebuilt only inside R5.5. The immutable R5.4 lock defines the approved 34-route/15-family/510-cell research universe; a family may emit zero historical candidates after causal evaluation without being deleted. Variants that explicitly set either rebuilt playbook to off are diagnostics only and cannot pass. Precomputed rebuilt candidates are immutable inputs to replay. R5.4 files, reliability promotion, portfolio constraints and replay logic remain unchanged.'}
