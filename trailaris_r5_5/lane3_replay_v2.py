...
def main():
    ...
    status={'state':'R5_5_LANE3_STRATEGY_REBUILD_V2_FULL_REPLAY_COMPLETE','evidence_class':'END_TO_END_CAUSAL_REBUILT_STRATEGY_REPLAY','variant_index':a.variant_index,'config':cfg,'r5_4_control':ctl,'candidate':cand,'delta_end_equity':end_delta,'fresh_delta_pct':fresh_delta,'rebuilt_candidates':int(len(rebuilt)),'rebuilt_raw_stats':stats(rebuilt),'gates':gates,'candidate_pass':all(gates.values()),'approved_scope':scope,'governance':'The two negative playbooks are replaced only in the R5.5 candidate. Precomputed rebuilt candidates are immutable inputs to every matrix replay. R5.4 files, reliability promotion, portfolio constraints and replay logic remain unchanged.'}
