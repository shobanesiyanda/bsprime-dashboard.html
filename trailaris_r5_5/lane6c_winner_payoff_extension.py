...
def extend_one(row,frame,runner_frac,target_r,trail_gap,max_extra_bars):
    ...
    for j in range(start,min(len(frame),start+int(max_extra_bars))):
        ...
        stop=ent+dr*floor_r*sd
        stopped=(lo<=stop+EPS if dr==1 else hi>=stop-EPS)
        if stopped:
        ...
        if targeted:
            rr=float(target_r);px=ent+dr*target_r*sd;xt=ts;break
        floor_r=max(floor_r,mfe-float(trail_gap))
        rr=(cl-ent)*dr/sd;px=cl;xt=ts
...
def extend_candidates(cands,fcache,runner_frac,target_r,trail_gap,max_extra_bars):
    ...
    for i,row in z.iterrows():
        ...
        if str(row.get('exit_reason',''))!='TARGET':continue
        if pd.notna(row.get('management_rule',np.nan)):continue
...
def main():
    ...
    z=cands.copy();ze=enrich_market_state(z,fcache);z['winner_extend_qualifies']=rule_mask(ze,rule).to_numpy()
    var=WinnerExtensionVariant(base,promote,a.runner_frac,a.target_r,a.trail_gap,a.max_extra_bars);cand,VW,VEV,VDE,VPR=eval_variant(var,data,z,opps,fcache,specs,100.0)
    selected_ext=int(VEV.get('winner_extension',pd.Series(index=VEV.index,dtype=object)).notna().sum()) if len(VEV) else 0
    ...
    s={'state':'R5_5_LANE6C_WINNER_PAYOFF_EXTENSION_COMPLETE','evidence_class':'END_TO_END_CAUSAL_FULL_REPLAY','rule_rank':a.rule_rank,'winner_rule':rule,'runner_frac':a.runner_frac,'target_r':a.target_r,'trail_gap_r':a.trail_gap,'max_extra_bars':a.max_extra_bars,'qualifying_candidates':int(z.winner_extend_qualifies.sum()),'extended_candidates_pre_replay':var.extended,'selected_extended_executions':selected_ext,'candidate_delta_r_before_portfolio_interaction':var.delta_r,'r5_4_control':ctl,'lane1b2_incumbent':inc,'candidate':cand,'delta_vs_1b2':float(cand['end_equity'])-INC_END,'alpha_gates':alpha,'alpha_candidate':bool(all(alpha.values())),'frontier_gates':frontier,'frontier_candidate':bool(all(frontier.values())),'promotion_candidate':False,'promotion_blockers':['NEW_UNSEEN_FORWARD_HOLDOUT_REQUIRED','TARGET_SERVER_BROKER_EXECUTION_CERTIFICATION_REQUIRED'],'scope':{'approved_routes':34,'strategy_families':15,'route_strategy_cells':510},'governance':'Only trades in a pre-Aug13 validated winner cohort can qualify. Extension activates only after an unmodified base trade has causally reached the existing 3R target; most profit is banked and only the configured runner fraction remains exposed behind a positive-R protective floor/trail. Initial risk is unchanged. Full portfolio chronology, capital lockup, factor exposure and later opportunity displacement are recomputed.'}
