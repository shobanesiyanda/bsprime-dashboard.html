...
# Configuration generation, validation and hierarchical reliability are frozen R5.4.
promote_for_week = frozen.promote_for_week

# Existing frozen Tier-B causal evidence requirements; no hindsight-derived thresholds.
DISPLACE_MAX_RISK_FRACTION = .0025
DISPLACE_MIN_QUALITY = .68
DISPLACE_MIN_EXPECTED_R = .10
DISPLACE_MIN_TRAILING_WIN = .45
DISPLACE_MAX_COST_R = .35
...
def replay_r5(cands, specs, start=100., feature_cache=None):
    ...
    c = cands.copy()
    c['decision_time'] = pd.to_datetime(c.decision_time, utc=True)
    ...
    openpos = {}; heap = []; events = []; dec = []; seq = 0
    factors = np.zeros(len(r4.FACTORS)); open_risk = 0.; open_margin = 0.; config_week_r = {}; asset_week_r = {}

    def mark(p, t):
        x = feature_cache.get(p['asset']) if feature_cache else None
        if x is None:
            return None
        lo = int(x.timestamp.searchsorted(pd.Timestamp(p['entry_time']), side='left'))
        hi = int(x.timestamp.searchsorted(pd.Timestamp(t), side='right'))
        if hi <= lo:
            return None
        z = x.iloc[lo:hi]; px = float(z.close.iloc[-1]); ent = float(p['entry_price']); sd = max(float(p['stop_distance']), 1e-12); dr = int(p['direction'])
        cur = (px-ent)*dr/sd - float(p.get('cost_r', 0))
    ...
    def close_until(t):
        ...
        while heap and heap[0][0] <= t:
            ...
            pnl = float(p['risk_cash']) * float(p['net_r']); before = mtm(p['exit_time']); balance += pnl
            factors -= r4.factor_vec(p['asset'], int(p['direction'])); open_risk = max(0, open_risk-float(p['risk_cash'])); open_margin = max(0, open_margin-float(p['margin']))
            eq = mtm(p['exit_time']); peak = max(peak, eq); dd = eq/peak-1
    ...
    def recycle_protected(new, t, u):
        ...
        _, pid, p, s = min(choices, key=lambda z:z[0]); openpos.pop(pid, None); rr = float(s['current_r']); before = mtm(t); pnl = float(p['risk_cash'])*rr; balance += pnl
        factors -= r4.factor_vec(p['asset'], int(p['direction'])); open_risk = max(0,open_risk-float(p['risk_cash'])); open_margin = max(0,open_margin-float(p['margin'])); eq = mtm(t); peak = max(peak,eq)
    ...
    def displacement_qualified(r):
        return (float(r.quality) >= DISPLACE_MIN_QUALITY and float(r.expected_r) > DISPLACE_MIN_EXPECTED_R and
                float(r.trailing_win_rate) >= DISPLACE_MIN_TRAILING_WIN and float(r.cost_r) <= DISPLACE_MAX_COST_R and
                float(r.get('reliability_score', 0.0)) > 0.0)

    def displace_adverse(new, t, u, new_risk, new_margin):
        """Atomic risk-neutral swap from adverse unprotected incumbent to stronger candidate."""
        nonlocal balance, peak, factors, open_risk, open_margin
        choices = []
        for pid, p in openpos.items():
            if bool(p.get('is_addon', False)): continue
            s = mark(p, t)
            if not s or s['floor'] >= 0 or float(s['current_r']) >= 0: continue
            u0 = float(p.get('entry_utility', p.get('rank_score', p.get('quality', 0))))
            if u <= u0 + .05: continue
            ...
            choices.append((u0, pid, p, s))
        if not choices: return None
        _, pid, p, s = min(choices, key=lambda z:z[0])
        # Admission is fully prevalidated before incumbent close. Incoming lot/risk/margin
        # remain fixed after close, so the substitution cannot expand risk or margin.
        openpos.pop(pid, None); rr = float(s['current_r']); before = mtm(t); pnl = float(p['risk_cash'])*rr; balance += pnl
        factors -= r4.factor_vec(p['asset'], int(p['direction'])); open_risk = max(0,open_risk-float(p['risk_cash'])); open_margin = max(0,open_margin-float(p['margin'])); eq = mtm(t); peak = max(peak,eq)
        # Unlike protected recycling, adverse realized R must enter the live weekly loss state.
        wk = r4.week_key(t); config_week_r[(wk,p['asset'],p['strategy'])] = config_week_r.get((wk,p['asset'],p['strategy']),0)+rr; asset_week_r[(wk,p['asset'])] = asset_week_r.get((wk,p['asset']),0)+rr
        events.append({**p,'timestamp':pd.Timestamp(t),'exit_time':pd.Timestamp(t),'exit_price':s['mark'],'net_r':rr,'net_pnl':pnl,'equity_before':before,'equity':eq,'balance_after':balance,'drawdown':eq/peak-1,'exit_reason':'ADVERSE_RISK_NEUTRAL_DISPLACEMENT_R5_5'})
        return pid

    for t, due in c.groupby('decision_time', sort=True):
        wk = r4.week_key(t)
        if week is None:
            week = wk; week_start = start
        else:
        ...
        for _, r in due.sort_values('rank_score', ascending=False).iterrows():
            dq = displacement_qualified(r)
            base_rf = float(r.get('risk_fraction', .005))
            rf = min(base_rf*rs, DISPLACE_MAX_RISK_FRACTION) if dq else base_rf*rs
            ...
            u=float(r.rank_score)-.20*(margin/max(eq,1e-12)); recycled=''; adverse_displaced=False

            if dq and reason in {'OPEN_RISK_BUDGET','MARGIN_BUDGET','FACTOR_DUPLICATION'} and u>=.12:
                pid=displace_adverse(r,t,u,risk,margin)
                if pid:
                    recycled=pid; eq=mtm(t); proj=factors+r4.factor_vec(r.asset,int(r.direction)); reason='SELECTED'; adverse_displaced=True
            if reason in {'OPEN_RISK_BUDGET','MARGIN_BUDGET','FACTOR_DUPLICATION'} and u>=.12:
                pid=recycle_protected(r,t,u)
                if pid:
            ...
            dec.append(dict(decision_time=t,campaign_id=r.campaign_id,asset=r.asset,strategy=r.strategy,strategy_family=r.strategy_family,promotion_tier=r.promotion_tier,selected=reason=='SELECTED',reason=reason,quality=float(r.quality),expected_r=float(r.expected_r),rank_score=float(r.rank_score),utility=u,recycled_incumbent_id=recycled,adverse_displaced=adverse_displaced,displacement_qualified=dq,equity=eq,week_growth=growth,risk_budget=risk_budget,lot=lot,risk_cash=risk,margin=margin,projected_open_risk_pct=100*(open_risk+risk)/max(eq,1e-12)))
            if reason=='SELECTED':
                p=r.to_dict(); p.update(lot=lot,risk_cash=risk,margin=margin,equity_at_entry=eq,balance_at_entry=balance,entry_utility=u,adverse_displaced=adverse_displaced); openpos[r.campaign_id]=p; factors=proj; open_risk+=risk; open_margin+=margin; seq+=1; heapq.heappush(heap,(pd.Timestamp(r.exit_time),seq,r.campaign_id))
