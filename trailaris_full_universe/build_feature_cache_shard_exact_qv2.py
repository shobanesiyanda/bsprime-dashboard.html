...
def main():
    ...
    for e in ev.get('evidence',[]):
        ...
        f=Path(e['file']);asset=str(e['canonical_instrument']);provider=str(e.get('provider_name',''));anchor=anchor_for(asset,provider)
        try:
            _,_,xf,_=r4.build_asset_components((asset,str(f)));need={'timestamp','high','low','close'}
            ...
            fn=f'FEATURE_{safe(asset)}.csv.gz';z.to_csv(a.outdir/fn,index=False,compression='gzip');accepted.append({**e,'feature_state':'PASS','universe_adapter_anchor':anchor,'feature_rows':len(z),'feature_file':fn})
        except Exception as ex:rejected.append({**e,'feature_state':'R4_EXTENDED_FEATURE_REJECT','feature_error':f'{type(ex).__name__}:{str(ex)[:500]}'})
        finally:
            try:f.unlink()
