...
def parse(d):
 ...
 out['taker_sell_base']=(out.volume-out.taker_buy_base).clip(lower=0);out['flow_imbalance']=(out.taker_buy_base-out.taker_sell_base)/out.volume
 return out
...
def build(asset,symbol):
 d=history(symbol);bars=len(d)
 d['flow5']=d.flow_imbalance.rolling(5,min_periods=3).mean();d['flow_z']=causal_z(d.flow5)
 trade_med=d.trades.shift(1).rolling(120,min_periods=40).median().replace(0,np.nan);d['activity']=np.log1p((d.trades/trade_med).clip(lower=0,upper=10));d['orderflow_score']=(d.flow_z*(.75+.25*np.tanh(d.activity))).clip(-4,4)
 d=d.dropna(subset=['orderflow_score']);d['asset']=asset;d['carry_score']=np.nan;d['funding_score']=np.nan;d['event_score']=np.nan;d['evidence_state']='VERIFIED_BINANCE_USDM_TAKER_FLOW_CAUSAL';d['source']='BINANCE_USDM_1M_KLINE_TAKER_BUY_BASE_VOLUME_ARCHIVE'
 ...
 return out,{'asset':asset,'symbol':symbol,'bars':bars,'factor_rows':len(out),'trigger_rows_abs_ge_070':int((out.orderflow_score.abs()>=.70).sum()),'first':str(out.timestamp.min()),'last':str(out.timestamp.max()),'state':'PASS' if len(out) else 'NO_FACTOR_ROWS'}
